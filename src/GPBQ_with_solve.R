...
t1 <- proc.time()
GPTime <- (t1 - t0)[[1]]
# compute the 1st integral

samples_GP_x <- randomLHS(10000, dim)
samples_GP_x_prime <- randomLHS(10000, dim)

integral_value <- 0
for (i in 1:10000) {
  integral_value <- integral_value + sum(exp(-.5 * rowSums(samples_GP_x - samples_GP_x_prime[i])^2))
}

integral_value <- integral_value/(10000^2)
...
print("sd")
integral_value - predictionGPBQ$standardDeviationGP
...
write.csv(results, file = "../results/genz/GP%s%sdim%s.csv" %--% c(whichGenz, whichGenz, dim),row.names=FALSE)