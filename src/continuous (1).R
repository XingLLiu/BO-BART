...
for (i in 1:500){
  ...
  value<-cont(t(candidateSet[index,]))
...
for (i in 1:500){
  ...
  integration<-mean(cont(CandidateSet))
...
real<- (adaptIntegrate(cont,lowerLimit = rep(0,1),upperLimit = rep(1,1))[[1]])^(dim)
percentageError<-abs((meanValue-real)/real[[1]])*100
percentageError2<-abs((meanValue2-real)/real[[1]])*100
percentageError3<-abs((meanValue3-real)/real[[1]])*100
