...
for (i in c(7)){
  for (j in c(1)){
    for (sequential in c("NoSequential")){
      ...
      for (num_cv in 1:5) {
        ...
        fileName <- paste(toString(genzFunctionName), 'Dim', toString(dim), sequential, "Gaussian_", toString(num_cv),'.csv', sep='')
        ...
        plotPath <- gsub(paste("../../Figures_step_10_jumps/", toString(whichGenz), "/", plotName, sep=""), pattern = "csv", replacement="jpg")
        ...
        plotPath_error <- gsub(paste("../../Figures_step_10_jumps/", toString(whichGenz), "/", plotName_error, sep=""), pattern = "csv", replacement="jpg")
        
        integrals <- read.csv(filePath, header=TRUE, sep=",", stringsAsFactors = FALSE)
        
        ymin <- min(c(predictionBART$meanValueBART - 2*predictionBART$BARTsd, predictionGPBQ$meanValueGP - 2*integrals$GPsd[0], predictionMonteCarlo$meanValueMonteCarlo[15:50]))
        ymax <- max(c(predictionBART$meanValueBART + 2*predictionBART$BARTsd, predictionGPBQ$meanValueGP + 2*integrals$GPsd[0], predictionMonteCarlo$meanValueMonteCarlo[15:50]))
    ...
    pdf(paste("../../Figures_step_10_jumps", "/combined_", toString(whichGenz), ".pdf", sep = ""), width = 8.5, height = 5)
  ...
  pdf(paste("../../gaussian_measure", "/", toString(whichGenz), ".pdf", sep = ""), width = 8.5, height = 10)
  plot(c(1, 10, 20), 
