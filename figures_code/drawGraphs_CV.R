...
# rsync -rav -e ssh --include '*/' --exclude='*.RData' ichpc:/rds/general/user/hbz15/home/BO-BART/results/genz/ results/new_genz
...
for (i in c(1,2,4,5,6,7)){
  for (j in c(1, 2, 3)){
    for (sequential in c("")){
      ...
      for (num_cv in 1:20) {
        ...
        analyticalIntegrals <- read.csv("results/new_genz/integrals.csv", header = FALSE)
        ...
        ymin <- min(c(predictionBART$meanValueBART - 2*predictionBART$standardDeviationBART, predictionMonteCarlo$meanValueMonteCarlo[1:20]))
        ymax <- max(c(predictionBART$meanValueBART + 2*predictionBART$standardDeviationBART, predictionMonteCarlo$meanValueMonteCarlo[1:20]))
    ...
    legend("topright", legend=c("BART-Int", "GP-BQ", "MI"),
           col=c("orangered", "dodgerblue", "chartreuse4"), cex=1.7, lty = c(1,1,1,1))
    plot_points <- seq(0, 50, 5)
  ...
  # genz_error <- mape_error[mape_error$X == genzFunctionName,]
  ...
  # pdf(paste("Figures", "/", toString(whichGenz), ".pdf", sep = ""), width = 8.5, height = 10)
  # plot(c(1, 10, 20), 
