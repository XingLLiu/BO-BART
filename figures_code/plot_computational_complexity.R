...
i=1
for (num in nums) {
  filename <- "computational_complexity/computational_complexity_stepDim10NoSequentialUniform_num%s.csv" %--% num
  df <- read.csv(filename)
...
# i=1
# for (num in nums) {
#   filename <- "computational_complexity/computational_complexity_stepDim20NoSequentialUniform_num%s.csv" %--% num
#   df <- read.csv(filename)
#   bart_times_20[i] <- df$runtimeBART
#   i <- i+1
# }

par(mfrow=c(1,1), pty="s")
plot(nums, gp_times, xlab=expression("n"[seq]), ylab="Time (s)", 
...
# points(nums, bart_times_20,lwd=6.8, col="black")
# m <- lm(bart_times_20~ nums)
# coeffs <-as.numeric(m$coefficients)
# abline(a=coeffs[1], b=coeffs[2], col="black")
